    ...
    def __init__(self,s_size,a_size,scope,trainer):
        with tf.variable_scope(scope):
            ...
            if scope != 'global':
                ...
                self.entropy = - tau * tf.reduce_mean(self.policy * tf.log(self.policy))
                self.advantages = tf.placeholder(shape=[None],dtype=tf.float32)
                #self.advantages_entropy = self.advantages + self.entropy
                self.log_res_output = tf.log(self.responsible_outputs)
                self.q_out = tf.reduce_sum(self.q * self.actions_onehot, axis=1)
                
                # actor loss
                # q loss
                self.q_policy = tf.reduce_sum(self.q * self.actions_onehot, axis=1)
                self.log_pi = self.log_res_output
                
                self.q_loss = 0.5 * tf.reduce_mean(tf.square(
                        self.values - tf.stop_gradient(self.q_policy - self.log_pi)))
                # policy loss
                #self.policy_loss = tf.reduce_mean(self.log_res_output*self.advantages_entropy) + self.entropy
                ...
                self.policy_loss = tf.reduce_mean(self.log_pi * tf.stop_gradient(
                        self.log_pi - self.q_policy + self.values)) + policy_regularization_loss
                
                # critic loss
                self.value_loss = 0.5 * tf.reduce_mean(tf.square(self.target_v - self.q_out))
